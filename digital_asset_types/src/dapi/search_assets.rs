...
pub async fn search_assets(
    ...
    let num_conditions: usize = search_assets_query
        .count_conditions();

    if num_conditions == 0 { todo!("return error") }
...
impl SearchAssetsQuery {
    ...
    pub fn count_conditions(&self) -> usize {
        ...
        if self.specification_verison.is_some() { num_conditions += 1; }
        if self.frozen.is_some() { num_conditions += 1; }
        if self.supply.is_some() { num_conditions += 1; }
        if self.compressed.is_some() { num_conditions += 1; }
        if self.compressible.is_some() { num_conditions += 1; }
        if self.nonce.is_some() { num_conditions += 1; }
        if self.royalty_amount.is_some() { num_conditions += 1; }
        if self.chain_data_id.is_some() { num_conditions += 1; }
        if self.burnt.is_some() { num_conditions += 1; }
        if self.seq.is_some() { num_conditions += 1; }
    ...
    pub fn conditions(&self) -> Condition {
        ...
        let conditions = match self.condition_type {
            ...
            None | Some(ConditionType::All) => Condition::all()
                .add_option(self.specification_verison.map(|x| asset::Column::SpecificationVersion.eq(x)))
                ...
                .add_option(self.royalty_amount.map(|x| asset::Column::RoyaltyAmount.eq(x)))
